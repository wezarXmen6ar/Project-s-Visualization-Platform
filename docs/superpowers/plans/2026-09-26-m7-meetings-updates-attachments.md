...
- **The person page, an "Accounts" card (الأحقيات):** type, expiry date (with the weekday), "renew within {n} days" / "جدّد خلال {n} يوماً" or "expired {n} days ago" / "انتهت منذ {n} أيام" (Arabic plurals), the reminder lead ("remind 30 days before" / "التذكير قبل 30 يوماً"), and the note. **+ Add account** takes a type, an expiry date, the reminder lead (default 30 days) and a note. **Renewed** on a row asks for the new expiry date and keeps the old one in the note ("Renewed on {date}; the expiry was {old}" / «جُدّدت في {date}، وكان تاريخ الانتهاء {old}»; appended to any existing note). Edit and Delete.
