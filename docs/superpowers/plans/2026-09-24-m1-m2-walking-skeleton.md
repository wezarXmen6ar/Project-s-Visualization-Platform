...
> **Renumbered on 2026-09-26 (second time):** Team statistics was inserted as M9, right after M8 (progress), so holds became M10, the layout pass M11, change requests and requirements M12, readiness M13, insight M14, playback M15, the sandbox M16, polish M17 and vendor companies M18. The M7 plan and the spec use the new numbers.

> **Reordered on 2026-09-26 (third time, user-approved review):** change requests and requirements (with the requirements bank) moved up to M9, straight after progress, and requirement readiness merged into it; team statistics became M10, holds M11 and the layout pass M12; stakeholder insight and polish for presenting merged into M13 Stakeholder presentation; playback is M14, the sandbox M15 and vendor companies M16. 16 milestones in all. The spec and the M7 plan use these numbers.
...
| M8 | Progress and decisions | Baseline 1 for every project; actual dates, % complete (each phase and sub-phase bar fills in a darker shade of its colour as it progresses, with the % in its hover details), pace indicator, late detection, **Mark done for every phase and sub-phase, done today or on an earlier date**, **four lateness warnings (overdue, behind schedule, project end at risk, reminder N days before the end) with the reason recorded**, early/late shift prompts with cause and responsibility; Delay a phase switches on; "Needs your decision" inbox; holidays with shift prompt; original plan and actual dates for past projects; first **Why did the end date move?** and **Where did the time go?** charts; **phase ownership (Technical / Business / Joint) with a distinct outline for business-owned phases**; **to-do history: updates linked to the meetings that discussed them ("To-dos discussed" in the meeting form), and to-do priority (Normal / Important / Urgent)**; **project summary, first version** (key dates incl. a new "date requested" field, on track or late, overall and development %, starred key documents), on both the presentation side and the project page; **a Department filter on the presentation dashboard and portfolio, for presenting to one business owner**; **storage protection: files included in the daily backup, a backup location of the user's choice, a storage page with a low-space warning, deleted files emptied after 90 days**; **status report PDF as slides, first version: cover, Summary and Timeline slides for one or several projects** |
| M9 | Requirements and change requests | Change requests, new baselines with dashed outlines, requirement fields on development sub-phases (source, date received), **requirements raised in a meeting**, the **requirements bank** (within scope / out of scope / not decided, scheduled later or sent for a change request), **different Gantt marks for added-later-within-scope and out-of-scope-with-a-change-request**, weights and the development % rule; change requests added to the stakeholder charts. **Plus requirement readiness (was a separate milestone):** incomplete/Ready with evidence, waiting clock, Clarification to-dos, start at risk, grey waiting segments; waiting time added to the stakeholder charts; the project summary gains change requests and requirements added later; status report slides gain change requests; status report gains a first **Development** slide (development sub-phases: developed / in development / next, readiness, scope origin) |
| M10 | Team statistics | Per person, for a chosen quarter or year: to-dos assigned, finished, % on time and late now; for tech people the phases and sub-phases worked on, working days and % finished on time; for project managers the projects managed (active and finished), their projects' to-dos, meetings held and % of projects on time; a sortable comparison table. Only delays caused by the technical team count against a person's on-time rate (from M8's recorded causes). **Admin only**, never on the presentation side. |
| M11 | Holds and project links | Record a hold, hatched hold bar with an arrow on the portfolio, Pause a project switches on, dependency arrows, **follow-on projects** (pre-filled wizard, carry-over, project family strip); first end-to-end browser tests; holds added to the stakeholder charts; **with the Department filter, faded context rows for other departments' projects that affected theirs, labelled with how** |
| M12 | Layout pass | Rework page layouts from the user's collected layout comments (spec §7): cards arranged side by side in a grid that uses the page width, sized by their content, instead of one long column; applied to every page built so far, in both Arabic (right to left) and English |
| M13 | Stakeholder presentation | Health model, ⭐ milestones and upcoming milestones, the finished charts with drill-down; **plus polish for presenting (was a separate milestone):** PDF export, presenter mode; **scope over time and requirement arrival markers, "Waiting on you" panel, plain-language Arabic summary, ⓘ explanations of terms, periodic stakeholder report**; the project summary gains milestones, health, the summary sentence and a one-page PDF; status report slides gain "Why it moved", the overview slide and finished styling; **requirements first: development board (developed / in development / next), readiness at a glance, scope origin on every requirement (original, added later, by change request, not approved), optional "if everything pending is approved" projection on the Gantt chart, short summaries**; **what the technical team delivered: finished sub-phases and requirements with planned vs actual dates, share on time, time saved, extra work absorbed** |
| M14 | Playback | Play single project and portfolio, pop-ups with documents, auto-pause; **saved playback stories with presenter notes, storyboard PDF** |
| M15 | What-if sandbox | Stacked hypothetical changes, hypothetical projects, anonymous resource picking, conflicts, price tag panel, save as proposed; **live "cost of a request" form and side-by-side trade-offs** |
| M16 | Vendor companies and contracts (nice to have) | A page per company: its people, our contract with it and the contract file, yearly amount paid, monthly salaries, and the budget surplus. Confidential. |
