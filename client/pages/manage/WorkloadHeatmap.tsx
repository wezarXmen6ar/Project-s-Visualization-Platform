...
import { heatLevel, isAccepted, isoWeek, LEVEL_KEY } from './heatmap';
...
export function WorkloadHeatmap({ loads, decisions, calendar, resources, selected, onSelect }: WorkloadHeatmapProps) {
  const { lang } = useLang();
  const t = useT();
  if (loads.length === 0) return <p className="muted">{t('heatmap.noPeople')}</p>;
  const weeks = loads[0].weeks.map((w) => w.weekStart);

  return (
    <div className="heatmap-scroll">
      <table className="heatmap heatmap-weeks" aria-label={t('resources.workload')}>
        <thead>
          <tr>
            <th scope="col" className="heatmap-person">{t('heatmap.person')}</th>
            {weeks.map((w) => (
              <th key={w} scope="col">
          ...
          {loads.map((person) => {
            ...
            return (
              <tr key={person.resourceId}>
                <th scope="row" className="heatmap-person"><Link to={`/manage/resources/${person.resourceId}`} dir="auto" data-user-content="">{person.name}</Link></th>
                {person.weeks.map((w) => {
                  ...
                  const leaveNote =
                    ranges.length > 0
                      ? t('heatmap.onLeaveRanges', { ranges: ranges.map((r) => leaveRangeLabel(lang, r)).join(t('common.listSeparator')) })
                      : '';
                  return (
                    <td key={w.weekStart}>
                      <button
                        ...
                        aria-label={t('heatmap.cell', {
                          name: person.name,
                          when: weekLabel(w.weekStart, calendar, lang),
                          load: Math.round(w.load),
                          available: Math.round(w.available),
                          level: t(LEVEL_KEY[level]),
                          leave: leaveNote,
                        })}
